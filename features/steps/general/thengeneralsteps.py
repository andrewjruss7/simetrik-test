...
def step_impl(context, expression):
    list_validation = context.browser.are_element_presents(
...
def step_impl(context, element_name, element_type, text_to_validate):
    ...
    text_element = GeneralComponents.get_text_element(
        context, element).rstrip()
    new_text_to_validate = join_words(
        split_and_replace_string(text_to_validate))
...
def step_impl(context, element_name, element_type, expression):
    element = transformation_helper(element_name, element_type)
    element_validation = GeneralComponents.check_exist_element(
        context, element)
    assertion = transform_validation(expression)
...
def step_impl(context, element_name, element_type, expression):
    ...
    assertion = transform_validation(expression)
    button_enabled = GeneralComponents.is_enabled_in_page(
